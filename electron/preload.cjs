/* Preload — exposes a tiny, audited API on `window.__daisyConquest` so the
 ...
 * to update status checks `window.__daisyConquest?.isDesktop` first — see
...
contextBridge.exposeInMainWorld("__daisyConquest", {
  isDesktop: true,
